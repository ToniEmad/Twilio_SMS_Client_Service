...
public class InboxServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // استرجاع الـ username من الجلسة
        ...
        try (Connection conn = PSQL.getConnection()) {
            // استرجاع msisdn الخاص بالمستخدم المسجل
            String getMsisdnQuery = "SELECT msisdn FROM customer WHERE username = ?";
            try (PreparedStatement msisdnStmt = conn.prepareStatement(getMsisdnQuery)) {
                ...
                if (msisdnRs.next()) {
                    String userMsisdn = msisdnRs.getString("msisdn");
