...
public class Customerlistservlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection conn = PSQL.getConnection()) {
            String query = "SELECT username, birthday, msisdn, email, job, address, twillio_sid, twillio_token FROM customer";
            try (PreparedStatement stmt = conn.prepareStatement(query);
                ...
                while (rs.next()) {
                    ...
                    out.println("<td>" + rs.getString("msisdn") + "</td>");
                    out.println("<td>" + rs.getString("email") + "</td>");
                    out.println("<td>" + rs.getString("job") + "</td>");
                    out.println("<td>" + rs.getString("address") + "</td>");
                    out.println("<td>");
                    out.println("<button class='btn btn-danger' onclick='deleteCustomer(\"" + rs.getString("msisdn") + "\")'>");
                    out.println("<i class='fas fa-trash'></i> Delete");
                    out.println("</button>");
                    out.println("</td>");
                    out.println("</tr>");
                }     
